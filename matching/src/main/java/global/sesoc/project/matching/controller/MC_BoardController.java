...
import global.sesoc.project.matching.DAO.Matching_likeDAO;
...
//좋아요
	@ResponseBody
	@RequestMapping(value="matching_like", method=RequestMethod.GET)
	public String matching_like(int boardnum, String title, String tripArea, HttpSession session){
		String id = (String) session.getAttribute("loginId");
		Matching_likeVO vo = new Matching_likeVO();
		vo.setId(id);
		vo.setBoardnum(boardnum);
		vo.setTitle(title);
		vo.setTripArea(tripArea);
		
		logger.debug("넘어오는 vo: {}", vo);
		
		Matching_likeVO check = null;
		check = dao3.matching_like_check(vo);
		if(check != null){				
			return "f";
		}
		else{
			dao3.matching_like(vo);
			return "t";
		}						
	}
